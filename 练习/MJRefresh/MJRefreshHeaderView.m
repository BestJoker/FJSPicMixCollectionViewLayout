...
- (void)setState:(MJRefreshState)state
{
    ...
    // 4.根据状态执行不同的操作
	switch (state) {
		case MJRefreshStateNormal: // 下拉刷新
        {
            // 刷新完毕
...
#warning 这句代码修复了，top值不断累加的bug
                    NSLog(@"mj_height == %f",self.mj_height);
                    NSLog(@"mj_contentInsetTop == %f",self.scrollView.mj_contentInsetTop);
                    if (self.mj_height == 64.0 && self.scrollView.mj_contentInsetTop && self.scrollView.mj_contentInsetTop >= 64) {
